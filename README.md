...
**bot 判定はクライアントごとに出方が違う**ため、1つが弾かれても残りを試します。
打ち切るのは、動画そのものが存在しない場合（削除・非公開）だけです。
また、ログイン要求が出ていても字幕トラックが付いてくることがあるため、
再生可否より先に字幕の有無を確認します。

すべて失敗した場合は、bot 判定を受けたかどうかで `BOT_CHECK` / `FETCH_BLOCKED` を出し分け、
どの経路がどう失敗したかを画面と `wrangler tail` のログに残します。
